...
def select_elements_starting_with_vowel(array)
  array.select{ |word| ['a', 'e', 'i', 'o', 'u'].include?(word[0])}
end
...
def separate_array_into_even_and_odd_numbers(array)
  even_array = []
  odd_array = []
  array.each { |n| n.even? ? even_array << n : odd_array << n}
...
def number_of_elements_that_are_palindromes(array)
  palindromes = []
  array.each{ |w| w == w.reverse ? palindromes << w : false}
  palindromes.length
end
...
def average_of_array(array)
  sum_of_array = (array.inject(:+)).to_f
  (sum_of_array / array.length).ceil
end
...
def check_a_string_for_special_characters(string)
  ...
  # /\W/ - a non-word charcter. /\w/ would - a word character
end
...
def is_a_3_dot_range?(range)
  range.last != range.max ? true : false
end
